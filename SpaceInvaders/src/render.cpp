...
void draw(const Scene& scene, const Camera& camera)
{
	...
	VkDescriptorSet material_set = state->frame_data[state->frame_index].descriptor_allocator->allocate(state->device, state->material_set_layout);
	{
		VkDescriptorImageInfo image_info{
			.sampler = state->sampler,
			.imageView = scene.player.texture->image_view,
